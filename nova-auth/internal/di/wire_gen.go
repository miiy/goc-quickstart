...
import (
	...
	"github.com/miiy/goc-quickstart/nova-auth/internal/service"
...
func InitApp(conf string) (*app.App, func(), error) {
	...
	authOptions := provideJwtAuthOptions(configConfig)
	jwtAuth := auth.NewJWTAuth(authOptions)
	...
	smsCodeRepository := repository.NewSMSCodeRepository(universalClient)
	miniProgram, err := provideMiniProgram(configConfig)
	if err != nil {
		return nil, nil, err
	}
	refreshTTL := provideRefreshTTL(configConfig)
	authServiceServer := service.NewAuthServiceServer(zapLogger, authRepository, tokenRepository, smsCodeRepository, jwtAuth, miniProgram, refreshTTL)
...
func provideRefreshTTL(config2 *config.Config) service.RefreshTTL {
	return service.RefreshTTL(time.Duration(config2.Jwt.RefreshExpiresIn) * time.Second)
}
