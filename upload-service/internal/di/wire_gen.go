...
import (
	...
	"github.com/miiy/goc-quickstart/upload-service/internal/service"
...
func InitApp(conf string) (*app.App, func(), error) {
	...
	storage := provideStorageConfig(configConfig)
	uploadRepository := repository.NewUploadRepository(gormDB)
	uploadServiceServer := service.NewUploadServiceServer(zapLogger, uploadRepository, storage)
	appApp := app.NewApp(configConfig, dbDB, universalClient, loggerLogger, uploadServiceServer)
	return appApp, func() {
	}, nil
}
...
func provideStorageConfig(config2 *config.Config) config.Storage {
