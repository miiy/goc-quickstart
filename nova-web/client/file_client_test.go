...
func TestFileClientUploadAvatar(t *testing.T) {
	client := &FileClient{HTTPClient: newTestHTTPClient(func(r *http.Request) (*http.Response, error) {
		...
		resp := testResponse(http.StatusOK, `{"user":{"id":"7","username":"alice","avatar":"http://cdn.test/files/a.png"}}`)
	...
	if int64(resp.Id) != 7 || resp.Avatar != "http://cdn.test/files/a.png" {
...
func TestFileClientUploadPostCover(t *testing.T) {
	client := &FileClient{HTTPClient: newTestHTTPClient(func(r *http.Request) (*http.Response, error) {
		...
		resp := testResponse(http.StatusOK, `{"file":{"id":"9","scene":"FILE_SCENE_POST_COVER","object_key":"post-covers/2026/06/a.png","url":"http://cdn.test/post-covers/a.png","mime_type":"image/png","size":"9"}}`)
		resp.Header.Set("Content-Type", "application/json")
		return resp, nil
	})}
	...
	if resp.URL != "http://cdn.test/post-covers/a.png" {
