...
func TestPostClientUpdatePostSendsFieldMask(t *testing.T) {
	client := &PostClient{HTTPClient: newTestHTTPClient(func(r *http.Request) (*http.Response, error) {
		...
		if body.Post.Title != "new title" || body.Post.Content != "new content" || body.Post.CoverURL != "http://cdn.test/cover.png" {
		...
		resp := testResponse(http.StatusOK, `{"post":{"id":"42","author_id":"7","author_name":"alice","title":"new title","cover_url":"http://cdn.test/cover.png","content":"new content"}}`)
	...
	if resp.CoverURL != coverURL {
		t.Fatalf("cover url = %q, want %q", resp.CoverURL, coverURL)
	}
}
