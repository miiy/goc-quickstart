...
func UploadAvatar(c *gin.Context) {
	...
	if _, err := userModule.userClient.UploadAvatar(c.Request.Context(), header.Filename, file); err != nil {
		if handleAuthError(c, err) {
			return
		}
		user, loadErr := loadCurrentProfile(c)
...
func clearSession(c *gin.Context) {
	session, err := userModule.sessionStore.New(c.Request, userModule.sessionName)
	if err != nil {
		_ = c.Error(err)
		return
	}
	...
	_ = userModule.sessionStore.Save(c.Request, c.Writer, session)
}
